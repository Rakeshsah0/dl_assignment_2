...
wb.init(project="CNN_Best_Model_Evaluation_Predictions", config={
    "epochs": epochs,
...
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
print(device)
model = CNN(input_shape, num_filters, filter_sizes, activation, pool_sizes, dense_neurons, num_classes,use_batch_norm,dropout_prob).to(device)
optimizers = {'adam': optim.Adam,'nadam': optim.Adam}
